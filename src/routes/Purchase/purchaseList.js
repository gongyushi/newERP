...
import { Table, Tabs, Form, Button, Modal, Radio, Select, message } from 'antd';
...
class PurchaseList extends React.Component {
  ...
  submitDemand = () => {
    if (this.state.radioValue === 1) {
      erpPost('demand/join', { attribute: JSON.stringify(this.state.purchaseArr) }, res => {
  ...
  addToOrder = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        erpPost('demand/join', values, res => {
          message.success(res.data.msg);
  ...
  renderProInfo = () => {
    ...
    return (
      <div className="proDataWrap">
        ...
        <Table rowKey="sku" dataSource={dataSource} columns={columns} rowSelection={rowSelection} />
      </div>
    );
  };
  render() {
      ...
      <div>
        ...
        >
          <RadioGroup onChange={this.onChangeRadio} value={this.state.radioValue}>
            <Radio value={1}>新增采购单</Radio>
            <Radio value={2}>添加采购单</Radio>
          </RadioGroup>
        </Modal>
        {/* 添加采购单 */}
