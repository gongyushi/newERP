...
import PurOperationLog from './PurOperationLog';
import PurBasicDetail from './PurBasicDetail';
...
export default class PurchaseDetail extends React.Component {
  ...
  getPurchaseDetail = () => {
    const { id } = this.state;
    erpPost('purchase/detail',{id}, res => {
      this.setState({
  ...
  handleSubmit = (key) => {
    ...
    const { reason, person_id, process, id, current_id_last_node } = this.state;
  ...
  render() {
    const { checkVisible, rejectVisible, loading, person, voidVisible, current_id_last_node, allData,
      submitVisible, title, detail, status, id, processList, personList, is_need_audit, activeKey } = this.state;
    return (
              ...
              Object.keys(allData).length && (
                <Tabs defaultActiveKey='1' activeKey={activeKey} onChange={this.toggleTab}>
                  ...
                  <TabPane tab='批次' key='2'>
                    <PurchaseBatch {...this.props} status={detail.status} />
                  </TabPane>
