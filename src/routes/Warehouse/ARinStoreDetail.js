...
import { Tabs,Table, Icon, message, Tooltip } from 'antd';
import { erpPost } from '../../services/ajax';
import styles from './ARinStoreDetail.less';
...
class ARinStoreDetail extends React.Component {
  ...
  getList = () => {
    const { id, page1 } = this.state;
    erpPost('warehouse-receipt/inbound-detail',{id,page:page1},res => {
      this.setState({
  ...
  getLogList = () => {
    const { id, page2 } = this.state;
    erpPost('warehouse-receipt/log-index',{id, page:page2},res => {
      this.setState({
